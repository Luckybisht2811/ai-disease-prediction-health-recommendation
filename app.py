...
# ===========================
app = Flask(__name__)
app.secret_key = "mysecretkey123"  # Secret key for session management
...
def helper(disease):
    """
    Returns description, precautions, medications, diet, and workout for a given disease
    """
    # Description
...
def get_predicted_value(patient_symptoms):
    """
    Converts symptoms into input vector and predicts disease using trained model
    """
...
def index():
    if 'user_id' not in session:
        return redirect(url_for('login'))
    return render_template("index.html", message=f"Welcome {session['user_name']}!")
...
def dashboard():
    ...
    conn = sqlite3.connect("users.db")
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM predictions WHERE user_id=?", (session['user_id'],))
    predictions = cursor.fetchall()
    conn.close()

    return render_template("dashboard.html", user_name=session['user_name'], predictions=predictions)
...
def predict():
    ...
    if request.method == 'POST':
        ...
        if not symptoms or symptoms.lower() == "symptoms":
            message = "Please either write symptoms or correct any spelling errors."
        ...
        return render_template(
            ...
            dis_des=dis_des,
            my_precautions=precautions_list[0],
